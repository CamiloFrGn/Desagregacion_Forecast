...
finHistoria = datetime.datetime(2023, 4 , 11)
...
#version = "ESTADISTICO_ABR_2023"
version = "CONSENSO_ABR_2023"
...
irregularidades  = pd.read_excel(r"C:\Users\jsdelgadoc\Documents\otros\Desagregacion_Forecast\app\datos\BaseIrregularidades.xlsx", engine="openpyxl")
...
def generardor_desagregacion(despachosSQL, calendarioLogistico, absorcionEstadistica, criterio_historia_reciente, inicioHistoria, finHistoria, nivel_volatilidad, volPais, pais , despachos_irregularidades, irr_nacional):
    ...
    if absorcionEstadistica == 1:
        ...
        archivo_volumen_ciudad = pd.read_excel(r'C:\Users\jsdelgadoc\Documents\otros\Desagregacion_Forecast\DatosAbsorcionPlantas\test' + pais + 'Ciudad.xlsx')
        ...
        DesagregacionPronosticoPlanta = pd.read_excel(r'C:\Users\jsdelgadoc\Documents\otros\Desagregacion_Forecast\DatosAbsorcionPlantas\test' + pais + '.xlsx')
    ...
    DesagregacionPronosticoPlantaDia = pd.merge(calendarioLogistico, DesagregacionPronosticoPlanta, on = 'key').drop(columns="key",axis=1)
    ...
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop(columns="DiaSemana_x",axis=1)
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop(columns="DiaSemana_y",axis=1)
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop(columns="Semana_Relativa_x",axis=1)
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop(columns="Semana_Relativa_y",axis=1) 
    ...
    matrizPPTO_Resultado = pd.merge(matrizPPTO_Resultado, DesagregacionPronosticoPlanta, on = 'Planta' ).drop(columns="key",axis=1)
    ...
    while( gapTotal > 1 ):
        ...
        DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop(columns="forecast_planta",axis=1)   
...
writer = pd.ExcelWriter(r"C:\Users\jsdelgadoc\Documents\otros\Desagregacion_Forecast\app\datos\Desagregacion_" + pais + "_" + pd.to_datetime("now").strftime("%Y-%m-%d-%H-%M-%S") + ".xlsx", engine='xlsxwriter')
df_result.to_excel( writer, sheet_name="Desagregacion", index=False , engine='openpyxl')
writer.close()
