# -*- coding: utf-8 -*-
"""
Created on Fri Feb 26 10:42:51 2021
...
def obtenerResultadoRNR(pais, version, mes, year):
    #Conectar con base sql y ejecutar consulta
    cursor = conectarSQL()
    try:
        cursor.execute("select * from SCAC_AT31_EscenariosForecast where Pais = ? and VersionForecast = ? and MONTH(FechaEntrega) = ? AND YEAR(FechaEntrega) = ? " , (pais, version, mes, year) )
        #obtener nombre de columnas
        names = [ x[0] for x in cursor.description]
...
vol_diario: dataset con el volumen total por dia (ya sea de la historia o del resultado de la RNR)
total_pais: numero del forecast a nivel nacional
"""
def generarDesagregacion( pais, despachosSQL, despachosSQL_historiaReciente, despachosSQL_absPlantas, absorcionEstadistica ,calendarioLogistico, vol_diario, total_pais, entrenamiento ):
    ...
    if absorcionEstadistica == True:  
        ...
        DesagregacionPronosticoPlanta = DesagregacionPronosticoPlanta[['Planta', 'absorcionPlanta' ]]
        DesagregacionPronosticoPlanta['M3ForecastPlanta'] = DesagregacionPronosticoPlanta['absorcionPlanta'] * total_pais
        ...
        DesagregacionPronosticoPlanta['totalPais'] = DesagregacionPronosticoPlanta['M3ForecastPlanta'].sum()
        DesagregacionPronosticoPlanta['absorcionPlanta'] = DesagregacionPronosticoPlanta['M3ForecastPlanta'] / DesagregacionPronosticoPlanta['totalPais']
        DesagregacionPronosticoPlanta = DesagregacionPronosticoPlanta[['Planta', 'absorcionPlanta', 'M3ForecastPlanta' ]]
    
        
    #Absorcion dia segun RNR
    if entrenamiento == True:
        vol_diario = pd.DataFrame( vol_diario.groupby(['FechaEntrega'])['totalEntregado'].sum() ).reset_index()
        vol_diario.rename(columns={'totalEntregado': 'Vol'}, inplace = True)
        
    vol_diario['total'] = vol_diario['Vol'].sum()
    vol_diario['absorcionDia'] = vol_diario['Vol'] / vol_diario['total']
    ...
    #join con absorcion por planta
    #DesagregacionPronosticoPlantaDia = pd.merge(DesagregacionPronosticoPlantaDia, DesagregacionPronosticoPlanta, how = 'left',  left_on=['Planta'], right_on=['Planta'] ).fillna(0)
    
    #join con absorcion por fecha
    DesagregacionPronosticoPlantaDia = pd.merge(DesagregacionPronosticoPlantaDia, vol_diario, how = 'left',  left_on=['Fecha de entrega'], right_on=['FechaEntrega'] ).fillna(0)
    
    #determino el volumen por planta por dia
    DesagregacionPronosticoPlantaDia['M3ForecastPlanta'] = DesagregacionPronosticoPlantaDia['absorcionPlanta'] * DesagregacionPronosticoPlantaDia['absorcionDia'] * total_pais
    ...
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop("FechaEntrega",1)  
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop("Vol",1)  
    DesagregacionPronosticoPlantaDia = DesagregacionPronosticoPlantaDia.drop("absorcionDia",1)  
...
yearTarget = 2021
mesTarget = 3
version_RNR = 'Escenario27'

inicioHistoria = datetime.datetime(2019, 5, 1) #'2013-05-01'
...
#este es el resultado del modulo Forecast_Nacional_RNR con menor MAPE
diario_nacional = obtenerResultadoRNR(pais, version_RNR, mesTarget, yearTarget)
...
for i in range (deltaYears):
    ...
    for j in range (12):
        ...
        #en este paso el primer parametro booleano deberia ser siempre TRUE !!!!!!!!!!! PILAS !!!!!!!!!!!!! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! --------------------------
        temp = generarDesagregacion(pais, despacho_anual_filtrados, despacho_mensual_filtrado, despacho_Abs, True ,calendarioLogistico, despacho_test_pivot, volPais, True) 
...
for i in range (deltaYears):
    ...
    for j in range (12):
        ...
        temp = generarDesagregacion(pais, despacho_anual_filtrados, despacho_mensual_filtrado, despacho_Abs, True ,calendarioLogistico, diario_nacional, volPais, False) 
